...
const static struct blecon_callbacks_t blecon_callbacks = {
    ...
    .on_scan_report = proximity_on_scan_report,
    .on_scan_complete = proximity_on_scan_complete,
};
...
int main(void)
{
    ...
    char self_device_id_str[BLECON_UUID_STR_SZ];

    blecon_get_identity(&_blecon, self_device_id);
    blecon_util_append_uuid_string(self_device_id, self_device_id_str);

    // All the devices we care about
    char* device_ids[] = {
        "2b3236c0-b327-41ed-a6db-9744c4210a30",
        "a52126b5-394a-4fba-b88a-7a45015c7e45",
        "af55209e-ed74-42f7-9f90-e4ab699b8935",
    };

    // Add device IDs (but skip ourselves)
    ARRAY_FOR_EACH(device_ids, i) {
        if(strcmp(device_ids[i], self_device_id_str) != 0) {
            bool success = proximity_add_device(device_ids[i]);
            if(!success) {
                // Out of monitoring slots
                break;
            }
        }
    }

    // Kick off initial scanning when event loop starts
    if(!blecon_scan_start(&_blecon, true, false, PROXIMITY_SCAN_TIME)) {
        printk("Failed to start scan for nearby devices\r\n");
    }
